...
public class OpConjureSword implements SpellAction {
    ...
    public Triple<RenderedSpell, Integer, List<ParticleSpray>> execute(List<? extends Iota> args, CastingContext context){
        ...
        ItemStack swordStack = HexconjuringItemRegistry.CONJURED_SWORD.get().getDefaultStack();
        swordStack.setCount(1);
        
        swordStack.getOrCreateNbt().putInt("durability", durability);
        
        ItemEntity sword = new ItemEntity(context.getWorld(), pos.getX(), pos.getY(), pos.getZ(), swordStack);
        
        return new Triple<RenderedSpell, Integer, List<ParticleSpray>>(new Spell(sword), mediaCost, List.of());
    ...
    public class Spell implements RenderedSpell{
        ...
        public Spell(ItemEntity sword){
            this.sword = sword;
        }

        public void cast(CastingContext ctx){
            ctx.getWorld().spawnEntity(sword);
        }
    }
