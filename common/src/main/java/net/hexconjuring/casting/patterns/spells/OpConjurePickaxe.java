...
public class OpConjurePickaxe implements SpellAction {
    /**
     * The number of arguments from the stack that this action requires.
     */
    public OpConjurePickaxe(){
    }
    ...
    public Triple<RenderedSpell, Integer, List<ParticleSpray>> execute(List<? extends Iota> args, CastingContext context){
        ...
        pickStack.getOrCreateNbt().putInt("durability", durability);
    ...
    public class Spell implements RenderedSpell{
        private ItemEntity pickaxe;
        public Spell(ItemEntity pickaxe){
            this.pickaxe = pickaxe;
        }

        public void cast(CastingContext ctx){
            ctx.getWorld().spawnEntity(pickaxe);
        }
    }
